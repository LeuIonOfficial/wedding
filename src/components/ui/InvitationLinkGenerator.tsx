...
export default function InvitationLinkGenerator({
	...
	const generateLinks = () => {
		...
		const links = names.map((name) => ({
			name,
			link: `${window.location.origin}${createGuestUrl(
				`/${lang}`,
				name
			)}`,
		}));
	...
	const downloadCSV = () => {
		...
		const rows = generatedLinks.map(
			(item) => `"${item.name.replace(/"/g, '""')}","${item.link}"`
		);
						...
						<button
							onClick={() => {
								if (
									...
									setSelectedGuest(
										selectedGuest || generatedLinks[0].name
									);
									setActiveTab("qrcode");
								}
							}}
					...
					{activeTab === "qrcode" && selectedGuestLink && (
						<div className="flex flex-col items-center">
							...
							<p className="mt-4 text-center font-medium">
								Scan to view personalized invitation for{" "}
								{selectedGuest}
							</p>
							...
							<div className="mt-6 text-sm text-gray-500">
								<p>
									Print this QR code and include it in
									physical invitations or share it digitally.
								</p>
							</div>
						</div>
					)}
